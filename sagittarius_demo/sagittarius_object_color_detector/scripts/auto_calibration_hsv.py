...
# 定义标定位姿点
calibration_points_z = 0.15
calibration_points = []
calibration_points.append(Point(0.18, 0,     calibration_points_z))
calibration_points.append(Point(0.18, 0.05,  calibration_points_z))
calibration_points.append(Point(0.25, 0.05,  calibration_points_z))
calibration_points.append(Point(0.25, -0.05, calibration_points_z))
calibration_points.append(Point(0.25, 0,     calibration_points_z))

# 定义拍照位姿
capture_point = Point(0.15, -0.002, 0.3)
capture_quaternion = Quaternion(-0.005, 0.719, 0.004, 0.695) 
...
for point in calibration_points:
    ...
    # 等待用户确认
    input_value = input("请将红色标定物放置到吸盘正下方，按任意键确认！")
    rospy.sleep(1)
