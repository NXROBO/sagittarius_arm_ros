...
void ObjectDetector::detection(cv::Mat image2hsv, HSV hsv_para, std::vector<cv::Rect> *box)
{
    ...
    cv::Mat mask;
    if (hsv_para.hmin > hsv_para.hmax) 
    {
        cv::Mat mask1;
        cv::Mat mask2;
        cv::inRange(image2hsv,
                    cv::Scalar(0, hsv_para.smin / float(255), hsv_para.vmin / float(255)),
        ...
        cv::inRange(image2hsv,
                    ...
                    cv::Scalar(360, hsv_para.smax / float(255), hsv_para.vmax / float(255)),
                    mask2);
        mask = mask1 | mask2;
    }
    else
